...
- `utl_file` is still a thin wrapper (16 handles, 1 KB lines, no directory objects); `external_call` takes a single int/float/string argument; REPL recompiles everything per input; reference counting has no cycle collection.
