...
- The custom SQL engine still has only three column types; views cannot be JOIN targets; index keys use only the first 36 bytes of strings. B-tree deletion rebalances, but the SQL layer still rebuilds indexes wholesale on row-chain rewrites, because a rewrite moves every row locator and not just the deleted ones.
