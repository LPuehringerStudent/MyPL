...
- The custom SQL engine has `int`, `float`, `string` and `bool` columns; the language's `date` and `timestamp` scalars still have no column type of their own (catalog format V6 reserves room for them). Views cannot be JOIN targets; B-tree deletion does not rebalance (indexes are rebuilt wholesale on row-chain rewrites); index keys use only the first 36 bytes of strings.
