...
- The custom engine binds `?var` values for static SQL, but not for cursors: `open c for select ... ?var` leaves the variable name in the SQL text and returns no rows, and `cursor c is select ... ?var` collects no parameters at all; the `dbms_sql` cursor API still substitutes `?N` into the SQL text at execute time instead of binding. Four `tests/test_phase6.c` tests (`bulk_collect_into_array_row`, `forall_*_with_scalar_array`) fail in `USE_SQLITE=0` builds because they pass `--db :memory:`, and would still fail on the custom engine for lack of an INSERT column list, multi-row `VALUES` and `SET col = <expr>`.
